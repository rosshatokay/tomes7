...
function addCustomFont(contents) {
  const doc = contents.document;
...
// app/javascript/reader.js
var { div: div2, button, span } = van_default.tags;
var Reader = class {
  /**
   * @type {import("vanjs-core").State<boolean>}
   */
  #locationsLoaded = van_default.state(false);
  #currentPercent = van_default.state();
  #book;
  #rendition;
  #elms = {
  ...
  async getBook() {
    await fetch(this.#epubEndpoint).then((response) => response.arrayBuffer()).then((data) => {
      this.#book = ePub(data);
      this.createReader();
    });
  }
  ...
  bindProgress() {
    van_default.add(
      this.#elms.progressIndicator,
      () => this.#locationsLoaded.val ? div2({ class: "text-secondary text-sm px-2" }, () => `${this.#currentPercent.val}%`) : div2({ class: "loading-spinner w-4 h-4 !border-1" })
    );
  }
  /**
  ...
  bindEvents() {
    this.#rendition.on("relocated", import_lodash.default.debounce((loc) => {
...
var ChaptersMenu = class {
  ...
  constructor(reader) {
    ...
    this.dropdown.setProps({
      onShow: () => {
        this.#renderChapters(this.dropdown.popper);
      }
    });
    this.reader = reader;
  }
  ...
  #renderChapters(popper) {
    ...
    const wrapper = div2(
      div2({ class: "p-2.5 px-4 border-b border-outline text-sm text-secondary" }, "Chapters"),
      div2(
    ...
    van_default.add(context, div2(
      () => !this.#chapters.val.length ? div2({ class: "flex-center text-sm text-secondary p-4 py-8" }, div2({ class: "loading-spinner w-6 h-6 border-2" })) : wrapper
    ));
  }
};
...
lodash/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=/assets/reader.js.map
