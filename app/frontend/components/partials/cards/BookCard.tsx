...
export const BookCard = ({ book, isMobileForced }: BookProps) => {
	...
	return (
		<div className={cn("bg-card rounded-xl flex relative hover:bg-black/5 dark:hover:bg-white/10 transition", isMobile ? "p-4 gap-4" : "p-5 flex-col gap-6")}>
			...
			<div className={cn("w-fit", !isMobile && "w-full flex items-center justify-center pt-2")}>
				<img src={book.cover}
					className={cn("min-w-20 w-20 rounded-[2px]", !isMobile && "w-3/5 min-w-initial")}
			...
			<div>
				<div className={cn("flex flex-col h-full py-3", !isMobile && "py-0")}>
					...
					<div className={cn("flex items-center gap-1 text-sm", isMobile ? "mt-auto" : "mt-1")}>
