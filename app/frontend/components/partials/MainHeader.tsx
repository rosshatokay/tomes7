...
export default function MainHeader({ title, description, content, badge, className }: Props) {
	return (
		<div className={cn("large-container flex-center flex-col h-[30vh] md:min-h-80 min-h-64", className)}>
			{badge && <Badge variant={"secondary"} className="mb-4 text-sm h-[initial]">{badge}</Badge>}
